...
/**
 * Applies elitist optimization to the best solutions.
 *
 * @param population The population of solutions.
 * @param fitnesses The fitness values of the solutions.
 * @param num_features The number of dimensions in the solutions.
 ...
 * @param evals The number of fitness evaluations.
 * @param max_evals The maximum number of fitness evaluations.
 * @param elitism_rate The rate of elitism.
 * @param gen The random number generator.
 */
template <typename Random>
void elitistOptimization(vector<vector<double>>& population, vector<double>& fitnesses, int num_features, double lower_bound, double upper_bound, int& evals, const int& max_evals, const double elitism_rate, Random& gen);
...
int main(int argc, char *argv[]) {
    ...
    for (int funcid = 1; funcid <= 30; funcid++) {
        for (int t = 0; t < T; t++) {
            ...
            cec17_init("tlboebsw", funcid, dim);
...
void tlbo(vector<vector<double>>& population, vector<double>& fitnesses, int dim, double lower_bound, double upper_bound, Random& gen, int max_evals) {
    ...
    while (evals < max_evals) {
        ...
        for (int i = 0; i < population.size(); i++) {
            // Calculate the new student
            vector<double> new_solution = population[i];

            // Compute x_ref using the actual student
            vector<double> x_ref(dim);
            double r1 = dis(gen);
            double r2 = dis(gen);

            for (int j = 0; j < dim; j++) {
                if ((best_solution[j] - new_solution[j] < 0) || (best_solution[j] - new_solution[j] == 0 && r2 < 0.5))
                    x_ref[j] = best_solution[j] + (pow(r1, 1.0 / (nm + 1)) - 1) * (best_solution[j] - lower_bound);
                else
                    x_ref[j] = best_solution[j] + (1 - pow(r1, 1.0 / (nm + 1))) * (upper_bound - best_solution[j]);
            }

            // Calculate modified student using the teacher and the mean student
            double tf = tf_dis(gen);
            double eta = dis(gen);

            for (int j = 0; j < dim; j++) {
                new_solution[j] += eta * (best_solution[j] - tf * mean[j] + (tf - 1) * x_ref[j]);
            }
...
template <typename Random>
void elitistOptimization(vector<vector<double>>& population, vector<double>& fitnesses, int num_features, double lower_bound, double upper_bound, int& evals, const int& max_evals, const double elitism_rate, Random& gen) {
    int num_elites = static_cast<int>(population.size() * elitism_rate);
    vector<int> elite_indices(population.size());
